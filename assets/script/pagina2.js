...
// variabili per gestione del colore
...
function startTimer() { //funzione per lo start del cronometro
    timerInterval = setInterval(() => {
      ...
      if (timeLeft === 0) {
        ...
        if (quizCompleted) { // quando il quiz è completo genera l'alert di completamento  
        calcolaRisposteEsatte();
        alert(`Hai completato il quiz con ${correctAnswers} risposte corrette! ${contatore}`);
        sessionStorage.setItem('contatore', contatore); 
        window.location.href = './pagina3.html';
...
function setRemainingPathColor(timeLeft) {
  ...
  function resetTimer() { //funzione per il reset del timer sia quando arriva a 0 che in caso di risposta con il tasto "procedi"
    onTimesUp();
    timePassed = 0;
    timeLeft = TIME_LIMIT;
